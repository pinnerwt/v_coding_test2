...
Create a skill that run all the benchmark (101 to 113) according to /bench-failure-triage (restart the server first, then run all of them), and write down 1. successful rate, 2. token usage (including cached/uncached, input/output) and 3. latency . The values will be served later for a python script that read the values in history, and generate evolution of these values with line chart. The skill will later check fail cases and check - if there is any loop in the trace, - if there is any hallucination in the trace (decisions not comming from the context), - tool misuse, - looping, - overuse of read() and finally, delta of the metrics from the last benchmark run. The skill will finally write down observations into observations.md (remove the old one first, and write to the new file)

Add parallelism for sessions. Use 5 sessions at the same time and emit queued event when connected.
