...
This file is the entire context the Phase 5b subagents need. Each subagent runs against ONE item body. The skill's main thread fans out, then merges.
...
## Future hazard — chunked bodies and the IBR-straddling-chunk problem

Today the main thread sends the full item body in a single Phase 5b call, so no IBR sentence can be split across calls. This will break when an item body is too large for one Haiku call:

- Item 1A (Risk Factors) at 100 KB+
- Item 8 (Financial Statements) at 60 KB+
- Item 15 EOF run-on at 1 MB+ on filings without an Item 16

When that happens, the main thread will have to chunk the body across N parallel Phase 5b calls. The naive non-overlapping split has a failure mode: an IBR sentence that straddles a chunk boundary will be missed by both calls (chunk N's tail ends mid-phrase as plain prose; chunk N+1's head begins mid-phrase as plain prose; neither sees `"incorporated by reference"` as a contiguous phrase).

**Mitigation when chunking is added:** overlap + resection.

- Each chunk N+1 carries the last ~500 chars of chunk N as a prefix (an IBR sentence is at most a few hundred chars, so a ~500-char overlap exceeds any plausible phrase length).
- At merge time, the main thread resections the overlap region: if either chunk classifies the overlap as `incorporated_by_reference`, that classification wins (IBR is the safer false positive — it preserves the audit trail; missing IBR loses signal).
- The cost is the duplicated input tokens per chunk boundary, which is small (~500 chars × N-1 boundaries) compared to the body itself.

This is documented here so future-you doesn't quietly drop chunking in without addressing it.
